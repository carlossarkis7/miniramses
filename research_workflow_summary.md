...
Choose or create a folder on your computer where you want to store all files for this project (e.g., GalaxyProjectSummer2025).
Navigate to that folder in Windows Explorer.
Right-click inside the folder and select “Git Bash Here” — this opens a terminal window pointed directly at your working directory.
...
Download the file output\_00011.tar from the link provided by Professor Teyssier.
Move the file into your project folder.
Extract the contents by running the following command in Git Bash or a terminal:
bash
Copy code
tar xvf output\_00011.tar
...
output\_00011/ – the simulation data
...
from miniramses import load\_cell

output\_path = "C:/path/to/project" # replace with your actual path
data = load\_cell(11, path=output\_path)
This command loads all the AMR cells from output\_00011, including positions (data.x), sizes (data.dx), and physical quantities like density (data.u[0]).
...
from miniramses import rd\_clump

clumps = rd\_clump(output=11, path=output\_path)
...
center\_coords = clumps["pos"][index]
This center\_coords value is later used to center the visualization on the main galaxy.
...
At this stage, a warning appeared:
...
Using the coordinates of the most massive clump identified in Step 3 (center\_coords), a zoomed-in region was extracted using rd\_cell():
from miniramses import rd\_cell

# Example: zoom with radius 0.005
c = rd\_cell(11, path=output\_path, center=center\_coords, radius=0.005)
Extracting Physical Fields
The rd\_cell() function returns a data object containing all the physical quantities within the zoomed region. From it, we extracted:
# Positions
...
The radius parameter in rd\_cell() was varied to explore the structure at different scales:
...
L = np.array([Lx, Ly, Lz])
L\_hat = L / np.linalg.norm(L) # normalized direction vector
